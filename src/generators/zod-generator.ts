...
export class ZodGenerator {
   ...
   */
  generateSchema(
    collection: DirectusCollectionWithFields,
    isCircularDependency: boolean = false
  ): string {
...
${omitFieldsString}
  ...
  private generateRepeaterSchema(field: DirectusField): string {
    ...
    const subFieldSchemas = repeaterFields.map((subField: any) => {
      ...
      const zodType = this.getZodTypeForSubField(subFieldType, subFieldMeta);

      // Handle nullable and optional
      if (
        subFieldMeta.interface === null ||
        subFieldMeta.interface === undefined
      ) {
          ...
          if (junctionRelations.length > 0) {
            ...
            if (fieldName === 'enables' || fieldName === 'disables') {
              ...
              if (!targetJunctionRelation) {
                targetJunctionRelation = junctionRelations.find(
                  (rel: any) =>
            ...
            if (targetJunctionRelation) {
              ...
              if (parts.length >= 2) {
                ...
                if (relatedPart) {
                  ...
                  const relatedExists = this.relationships.some(
                    (rel: any) =>
                      rel.collection === relatedCollection ||
                      rel.related_collection === relatedCollection
                  );
   ...
   */
  private generateRelationshipDebugInfo(
    collection: DirectusCollectionWithFields
  ): string {
    const relationFields = collection.fields.filter(field =>
...
`;
    ...
    const schemaDefinitions = schemas.map(schema => schema.schema).join('\n\n');
